...
#    dx[:,i] = J.dot(v[:,i])
    ...
    # e      = x_ref[:,i] - x[:,i]
    # de     = dx_ref[:,i] - dx[:,i]
    # f_star = f_ref[:,i] + kd*( f_ref[:,i] - f[:,i] ) # + (f_ref[:,i] - f[:,i])*conf.dt

    # ddq0 = kp * ( conf.q0 - q[:,i] ) - kd*v[:,i]

    # tau[:,i] = h + J.T.dot(kp * e + kd * de - f_star) + M.dot(np.identity(6) - J.T.dot(J)).dot( ddq0 )
    tau[:,i] = h - J.T.dot(f_ref[:,i] - ddx_des[:,i]) 
