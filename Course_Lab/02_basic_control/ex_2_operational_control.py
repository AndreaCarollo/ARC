...
#    dx[:,i] = J.dot(v[:,i])
    ...
    # flag_motion_control
    # 1 - DIRECT OPERATIONAL MOTION CONTROL
    # 2 - INV KINEMATIC + INV DYNAMIC
    flag_motion_control = 1

    if flag_motion_control == 1:
        Lambda = inv( J.dot(inv(M)).dot(J.T) ) 
        mu     = J.dot(inv(M).dot(h)) - dJdq
        N_null = np.identity(6) - J.T.dot(Lambda).dot(J).dot(inv(M))

        ddq_des = kp*( conf.q0 - q[:,i]) + kd*( - v[:,i] )
        tau_0   = M.dot(ddq_des) + h

        tau[:,i] = J.T.dot( inv(J.dot(inv(M)).dot(J.T)).dot(ddx_des[:,i]) + Lambda.dot( mu ) ) + N_null.dot(tau_0)
    
    elif flag_motion_control == 2:
        N_null   = np.identity(6) - pinv(J).dot(J)
        ddq_des  = kp*( conf.q0 - q[:,i]) + kd*( - v[:,i] )
        ddq_d    = pinv(J).dot(ddx_des[:,i] - dJdq) + N_null.dot(ddq_des)
        tau[:,i] = M.dot(ddq_d) + h
