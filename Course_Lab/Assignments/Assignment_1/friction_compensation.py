...
class PositionControl:
    ...
    def __init__(self, kp, kd, ki, dt, motor_params, tanh_fric_comp, tanh_gain=0.0):
        ...
        self.kd = kd
        self.dt = dt
        tau_coulomb = motor_params.tau_coulomb_gear / \
            motor_params.N + motor_params.tau_coulomb
    ...
    def compute(self, q, dq, q_ref, dq_ref):
        ...
        if(self.tanh_fric_comp):
            u_friction = np.tanh(self.tanh_gain * dq) * self.i_coulomb
        else:
            u_friction = np.sign(dq) * self.i_coulomb

        err = (q_ref - q)
        err_dot = (dq_ref - dq)
        self.error_integral += dt * err

        u = self.kp * err + self.kd * err_dot + \
            self.ki * self.error_integral + u_friction
...
def run_simulation(N, dt, kp, kd, ki, motor_params, tanh_fric_comp, tanh_gain):
    ...
    controller = PositionControl(
        kp, kd, ki, dt, motor_params, tanh_fric_comp, tanh_gain)
    ...
    for i in range(N):
        ...
        res.q[i] = motor.q()
        res.dq[i] = motor.dq()
        # add some random noise to the velocity estimation
        res.dq_est[i] = res.dq[i] + 0.25*(random()-0.5)

        # compute motor current with controller
        res.current[i] = controller.compute(
            res.q[i], res.dq_est[i], res.q_ref[i], res.dq_ref[i])
...
if __name__ == '__main__':
    ...
    # res_sign = run_simulation(
    #     N, dt, kp, kd, ki, motor_params, tanh_fric_comp=False, tanh_gain=0.0)
    # print("Friction compensation with tanh(0.01*dq)")
    # res_tanh_1 = run_simulation(
    #     N, dt, kp, kd, ki, motor_params, tanh_fric_comp=True, tanh_gain=0.01)
    # print("Friction compensation with tanh(2*dq)")
    res_tanh_2 = run_simulation(
        N, dt, kp, kd, ki, motor_params, tanh_fric_comp=True, tanh_gain=2.0)
    # print("Friction compensation with tanh(10*dq)")
    # res_tanh_10 = run_simulation(
    #     N, dt, kp, kd, ki, motor_params, tanh_fric_comp=True, tanh_gain=10.0)

    my_kp = 2
    my_kd = 2
    my_ki = 0.1
    print("Friction compensation with tanh(10*dq)")
    res_tanh_2_mypid = run_simulation(
        N, dt, my_kp, my_kd, my_ki, motor_params, tanh_fric_comp=True, tanh_gain=2.0)

    # plot_stuff(res_sign,    "sign(dq)")
    # plot_stuff(res_tanh_1,  "tanh(0.01*dq)")
    # plot_stuff(res_tanh_2,  "tanh(2*dq)")
    # plot_stuff(res_tanh_10, "tanh(10*dq)")
    # plot_stuff(res_tanh_2_mypid, "tanh(2*dq) my pid")
