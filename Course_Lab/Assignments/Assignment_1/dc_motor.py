import sys
import numpy as np
import numpy.linalg as la        # contain the linear algebra operators
...
if __name__=='__main__':
    ...
    np.set_printoptions(precision=1, linewidth=200, suppress=True) # set precision to print arrays
    ...
    V_w =  10.0       # frequency of sinusoidal motor voltage
    V_A =  0.0       # amplitude of sinusoidal motor voltage
...
''' The realationship is aproximately linear, 
    excluding the first phase in which there is a peak of current due to intertia of the motor '''
# How is the relationship between voltage and speed? 1) Linear, 2) Approximately linear, 3) Quadratic
''' Same as the curretn voltage -> approximately linear, excluding first moment due to inertia'''
