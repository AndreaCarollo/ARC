...
class PositionControl:
    ...
    def compute(self, q, dq, q_ref, dq_ref):
        # u = kp*(q_ref-q) + kd*(dq_ref-dq) + ki*integral(q_ref-q)
        self.error_integral += (q_ref - q)*self.dt
        error_position = q_ref - q
        error_velocity = dq_ref - dq
        u = self.kp*error_position + self.kd*error_velocity + self.ki*self.error_integral
        return u
...
if __name__ == '__main__':
    ...
    # from dc_motor_complete import Motor, get_motor_parameters
