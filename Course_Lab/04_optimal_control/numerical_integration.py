...
class Integrator:
    ...
    def integrate(self, ode, x_init, U, t_init, dt, ndt, N, scheme):
        ...
        '''
        n = x_init.shape[0]
        t = np.zeros((N*ndt))*np.nan
        x = np.zeros((N*ndt,n))*np.nan
        ...
        if(scheme=='RK-1'):
            ...
            raise Exception('Integration method not implemented yet!')
            ...
            for i in range(x.shape[0]-1):
                hh = 0.5*h
                ii = int(np.floor(i/ndt))
                k1 = ode.f(x[i,:n]        , U[ii,:], t[i])
                k2 = ode.f(x[i,:n] + hh*k1, U[ii,:], t[i] + hh)
                k3 = ode.f(x[i,:n] + hh*k2, U[ii,:], t[i] + hh)
                k4 = ode.f(x[i,:n] +  h*k3, U[ii,:], t[i] +  h)
    ...
    def integrate_w_sensitivities_u(self, ode, x_init, U, t_init, dt, N, scheme):
        ...
        if(scheme=='RK-1'):
            ...
            for i in range(x.shape[0]-1):
                hh = 0.5*h
                (k1, k1_x1, k1_u) = ode.f(x[i,:n]        , U[i,:], t[i],      jacobian=True)
                (k2, k2_x2, k2_u) = ode.f(x[i,:n] + hh*k1, U[i,:], t[i] + hh, jacobian=True)
                (k3, k3_x3, k3_u) = ode.f(x[i,:n] + hh*k2, U[i,:], t[i] + hh, jacobian=True)
                (k4, k4_x4, k4_u)  = ode.f(x[i,:n] + h*k3, U[i,:], t[i] + h,  jacobian=True)
                dx[i,:]   = (k1 + 2*k2 + 2*k3 + k4)/6.0
                x[i+1,:]  = x[i,:] + h*dx[i,:]
                t[i+1]    = t[i] + h

                k1_x = k1_x1
                k2_x = k2_x2*(I + 0.5*h*k1_x)
                k3_x = k3_x3*(I + 0.5*h*k2_x)
                k4_x = k4_x4*(I +     h*k3_x)

                phi_x = I + h/6.0*(k1_x + 2*k2_x + 2*k3_x + k4_x)
                phi_u = h/6 * ( k1_u + 2*k2_u + 2*k3_u + k4_u)
...
#    system = 'stiff-diehl'
    ...
    integrators += [{'scheme': 'RK-1',      'ndt': 24}]
    ...
    U = np.zeros((N,m));        # control input trajectory
    x_coarse = {}   # state trajectory sampled on the coarse time grid (corresponding to dt)
    x_fine = {}     # state trajectory sampled on the fine time grid (corresponding to dt/ndt)
    t_fine = {}     # fine time grid
    dx = {}         # state derivative with respect to time
