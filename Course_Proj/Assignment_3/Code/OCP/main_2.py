...
considering the contact of the end effector 
with the floor and then use the output of the 
OCP as reference for a Reactive Control that
has to provide a target force at the end effector
"""
...
if __name__=='__main__':
    ...
    from utils.robot_loaders import loadUR_urdf
    ...
    from utils.cost_functions import OCPRunningLockEERot, OCPRunningCostMinJointVel, OCPRunningOrthTool, OCPRunningCostQuadraticControl
    ...
    for name in conf.contact_frames:
        contact_points += [ContactPoint_ODE(robot.model, robot.data, name)]
    for name in conf.contact_surface_name:
        contact_surfaces += [ContactSurface_ODE(conf.contact_surface_name, conf.contact_surface_pos,
    ...
    q_00 = problem0.X[0,:conf.nq]
    x0 = np.concatenate((q_00, np.zeros(6)))       # initial states

    problem = SingleShootingProblem('ssp', ode_wc, x0, dt, N, conf.integration_scheme, simu)
    problem.visu = conf.SHOW_ITERATION_OCP_2

    # Compute the initial condition in task space given an initial condition in joint space
    robot.computeAllTerms(q_00, np.zeros(6))
    H = robot.framePlacement(q_00, problem.frame_id, False)
    p_0 = H.translation
    ...
    # Add the costs to the problem - the costs are defined in the cost_functions file
    # Final costs :
    final_fixP = OCPFinalCostFixedPoint(robot, p_des, conf.weight_pos_2)    # Final cost penalizing the final position in task space
    problem.add_final_cost(final_fixP)
   
    zero_vel = OCPFinalCostZeroVelocity(robot, conf.weight_vel_2)           # Final cost penalizing the final velocity in joint space
    problem.add_final_cost(zero_vel)

    # Running costs :
    traj_cost = OCPRunningCostTrajectory(robot, traj, dt)                   # Cost to follow a desired trajectory - class Trajectory - in task space
    problem.add_running_cost(traj_cost, conf.weight_traj_2)

    q_post = q_00
    post_js = OCPRunningCostPosturalTask(robot, q_post, dt)                 # Cost to keep a desired configuration during the task
    problem.add_running_cost(post_js, conf.weight_post_2)

    min_j_vel = OCPRunningCostMinJointVel(robot, dt)                        # Cost to minimize the joint velocity during the trajectory
    problem.add_running_cost(min_j_vel, conf.weight_vel_r_2)
    ...
    for i in range(N+1):
        ...
        x_real_wc[i,:] = H.translation
        # x_traj[i,:] = traj.pos
        err += norm(traj.pos - x_real_wc[i,:])
    err_mean = err/(N+1)
    ...
    for i in range(0, N*ndt, 1):
        if i%ndt == 0:
            q_ref[:,i] = problem.X[ii,:robot.nq].T
            v_ref[:,i] = problem.X[ii,robot.nq:].T
            ii += 1
        else:
            k = (i-(ii-1)*ndt)/ndt
            dX = problem.X[ii,:] - problem.X[ii-1,:]
            q_ref[:,i] = (problem.X[ii-1,:robot.nq] + dX[:robot.nq]*k).T
            v_ref[:,i] = (problem.X[ii-1,robot.nq:] + dX[robot.nq:]*k).T
    ...
    for i in range(0, N*ndt):
        ...
        J6 = robot.frameJacobian(q[:,i], problem.frame_id, False)
        ...
        H        = robot.framePlacement(q[:,i], problem.frame_id, False)
        x[:,i]   = H.translation
    ...
    if (conf.PLOT_TRACK_OCP):
        max_plots = 3
        if(x_real_wc.shape[0] == 1):
            nplot = 1
            (ff, ax) = plut.create_empty_figure()
            ax = [ax]
        else:
            nplot = min(max_plots, x_real_wc.shape[0])
            (ff, ax) = plut.create_empty_figure(nplot, 1)
        i_ls = 0
        ax[0].set_title("Trajectories")
        for i in range(len(ax)):
            ls = linestyles[0]
            ax[i].plot(time1, x_real_wc[:, i], ls, label='traj OCP wc', alpha=0.7)
            ls = linestyles[1]
