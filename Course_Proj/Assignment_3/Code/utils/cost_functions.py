...
#         tmp_1 = 0.5*(x[1] + x[2] - x[3] - 0.5*np.pi)**2
#         tmp_2 = 0.5*(x[4] - 0.5*np.pi)
...
#         grad_x = np.zeros(x.shape[0])
#         grad_x[1] = (x[1] + x[2] - x[3] - 0.5*np.pi)
#         grad_x[2] = (x[1] + x[2] - x[3] - 0.5*np.pi)
#         grad_x[3] = - (x[1] + x[2] - x[3] - 0.5*np.pi)
#         grad_x[4] = (x[4] - 0.5*np.pi)
...
# class OCPRunningOrthTool:
#     ''' Cost function to keep the end effector orthogonal to the plane using pin.log3()
#     '''
#     def __init__(self, robot, frame_id, dt):
#         self.dt = dt
#         self.robot = robot
