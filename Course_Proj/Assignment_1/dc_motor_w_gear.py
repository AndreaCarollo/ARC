...
class MotorWGear:
    ...
    def simulate(self, i, method='time-stepping'):
        dq_j = self.x[1]
        self.current = i
        self.voltage = self.R * i + self.K_b * dq_j / self.N
        self.tau_m = self.K_b * i
        
        # I*dq_j' + dt*tau_c = I*dq_j + dt*(N*tau_m - b*dq_j)

        # dq' = dq + dt * ( N*tau_m - tau_cg - N*tau_cm - (b_j+N^2*b_m)*dq )/( I_j + N^2*I_m )
        
        # I = I_j + N^2 * I_m ; b = b_j + N^2 * b_m ; tau_f = tau_cg + N * tau_cm
        # dq' = dq + dt * ( N*tau_m - tau_f - b*dq )/I

        # s = dt * (tau_m - b * dq) + I * dq

        I = self.I_j + self.I_m * (self.N)**2
        b = self.b_j + self.b_m * (self.N)**2

        s = self.dt * ( self.N * self.tau_m - b * dq_j ) + I * dq_j

        tau_0 = self.tau_c_g + self.tau_c_m * self.N


        # compute friction torque
        if(method=='time-stepping'):
            if np.abs(s/self.dt) <= tau_0:
                self.tau_f = s/self.dt
            else:
                self.tau_f = tau_0*np.sign(s)

        elif(method=='standard'):
            if (dq_j == 0):
                if (np.abs(self.tau_m) < tau_0):
                    self.tau_f = self.tau_m
                else:
                    self.tau_f = tau_0 * np.sign(self.tau_m)
            else:
                self.tau_f = tau_0 * np.sign(dq_j)
        ...
        self.x[1] = ( s - self.dt * self.tau_f )/I
...
#    params.N = 20
    ...
    for i in range(N):
        ...
        # motor.simulate(current[i],'standard')
